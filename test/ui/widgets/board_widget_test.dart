...
void main() {
  ...
  testWidgets(
      'BoardWidget onMovedTask callback works correctly - move from left to right',
      (WidgetTester tester) async {
    // Arrange: Create a BoardProvider with a board containing columns.
    final boardProvider = BoardProvider();
    boardProvider.board = Board.fromConfig({
    ...
    // Assert: The task should be moved to the right column.
    final taskCardsInToDo = tester
        .widgetList<TaskCard>(find.descendant(
            of: find.ancestor(
                of: find.text('To Do'), matching: find.byType(Column)),
            matching: find.byType(TaskCard)))
        .toList();
    final taskCardsInDoing = tester
        .widgetList<TaskCard>(find.descendant(
            of: find.ancestor(
                of: find.text('Doing'), matching: find.byType(Column)),
            matching: find.byType(TaskCard)))
        .toList();

    expect(taskCardsInToDo, isEmpty);
    expect(taskCardsInDoing, hasLength(1));
  });

  testWidgets(
      'BoardWidget onMovedTask callback works correctly - move from right to left',
      (WidgetTester tester) async {
    // Arrange: Create a BoardProvider with a board containing columns.
    final boardProvider = BoardProvider();
    boardProvider.board = Board.fromConfig({
